...
public class Tokenizer {
    ...
    public SequencedCollection<Token> tokenize(@NotNull final String source) {
        var tokens = (SequencedCollection<Token>) new LinkedList<Token>();
        ...
        while (!stop) {
            state = switch (state) {
                ...
                case State.Keyword _ -> switch (CharCode.from(character)) {
                    ...
                    default -> switch (KEYWORD.from(char_buff.toString())) {
                        case Ok(var keyword) -> new State.EndToken(new Token.Keyword(keyword));
                        case Err(_) -> new State.EndToken(new Token.Unknown(char_buff.toString()));
                    };
                };

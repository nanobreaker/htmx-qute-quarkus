...
@ApplicationScoped
@Unremovable
public class AuthenticationCompletionHandler implements io.quarkus.oidc.AuthenticationCompletionAction {
    ...
    public Uni<Void> action(final AuthenticationCompletionContext context) {
        ...
        var username = principal.<String>getClaim(Claims.upn);
        var session = principal.<String>getClaim("sid");
        var iat = Instant.ofEpochSecond(principal.getClaim(Claims.iat));

        var command = new UserCommand.Authenticate(id, username, session, iat);
        ...
        return eventBus
                ...
                .invoke(result -> {
                    switch (result) {
                        case Ok(_) -> Log.infof("User %s authenticated", username);

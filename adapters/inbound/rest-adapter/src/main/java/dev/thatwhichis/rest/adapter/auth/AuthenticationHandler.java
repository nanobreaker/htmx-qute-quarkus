...
@ApplicationScoped
@Unremovable
public class AuthenticationHandler implements AuthenticationCompletionAction {

    private final EventBus eventBus;

    @Inject
    public AuthenticationHandler(EventBus eventBus) {
    ...
    public Uni<Void> action(final AuthenticationCompletionContext context) {
        ...
        return eventBus
                ...
                .invoke(result -> {
                    switch (result) {
                        case Ok(_) -> Log.infof("User authenticated [id: %s]", id);
                        case Err(var error) -> Log.errorf("User authentication failed [id: %s, error: %s]", id, error.describe());
                    }
                })
                .replaceWithVoid();
    }
